...
<body>
  ...
  <center>
    <h2>Laporan Penjualan Toko Toko Cyber Sport</h2>
  </center>
      ...
      <tbody>
        ...
        ?>
         <tr>
          ...
          <td>
            <?php 
             $xx = mysqli_query($koneksi,"SELECT sum(invoice_total_bayar) as total FROM invoice WHERE  date(invoice_tanggal) >= '$tgl_dari' AND date(invoice_tanggal) <= '$tgl_sampai'");
             $x = mysqli_fetch_assoc($xx);
             echo " Rp.". number_format($x['total']);
            ?>
          </td>
        </tr>
      </tbody>
